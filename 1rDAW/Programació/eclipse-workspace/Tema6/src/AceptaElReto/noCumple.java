...
public class noCumple {
	...
	do {
		...
		if (diaNaix == diaActual && mesNaix == mesActual) {
			...
			noCumple = ((anyActual - anyNaix) * 364) + (mesos[mesNaix-1] - diaNaix) + diaActual;
		...
		else if ((mesNaix > mesActual) || (mesNaix == mesActual && diaNaix > diaActual)) {
			noCumple = ((anyActual - anyNaix - 1) * 364) + (mesos[mesNaix-1] - diaNaix) + diaActual;
		...
		else if (mesNaix == mesActual && diaNaix < diaActual) {
			noCumple = ((anyActual - anyNaix) * 364) + (diaActual - diaNaix);
			System.out.println(noCumple);
		}
		
	}while (diaNaix != 0 && mesNaix != 0 && anyNaix != 0 && diaActual != 0 && mesActual != 0 && anyActual != 0);
