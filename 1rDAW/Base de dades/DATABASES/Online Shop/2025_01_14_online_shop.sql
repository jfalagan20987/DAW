...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 14, 2025 at 07:42 PM
...
--
-- Stand-in structure for view `categories_views`
...
(10, 'Jalen', 'Williams', '41213377W', 'jdub@gmail.com', '+34 601227399', '2002-12-31');
...
--
-- Stand-in structure for view `total_spent_customers_view`
-- (See below for the actual view)
--
CREATE TABLE `total_spent_customers_view` (
`customer_id` int(11)
,`forename` varchar(100)
,`surname` varchar(100)
,`total_spent` decimal(42,2)
...
--
-- Structure for view `total_spent_customers_view`
--
DROP TABLE IF EXISTS `total_spent_customers_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `total_spent_customers_view`  AS SELECT `c`.`customer_id` AS `customer_id`, `c`.`forename` AS `forename`, `c`.`surname` AS `surname`, sum(`o`.`subtotal`) AS `total_spent` FROM (`order_view` `o` join `customers` `c` on(`o`.`customer_id` = `c`.`customer_id`)) GROUP BY `c`.`customer_id` ORDER BY sum(`o`.`subtotal`) DESC ;
