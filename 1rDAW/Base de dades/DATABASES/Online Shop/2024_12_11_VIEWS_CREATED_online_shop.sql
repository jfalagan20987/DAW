...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 11-12-2024 a las 21:07:49
...
--
-- Estructura Stand-in para la vista `categories_views`
...
(1, 1, 10, 1, 1089.00, '2024-12-09 08:52:41'),
(1, 1, 15, 2, 28.99, '2024-12-09 08:52:41'),
(2, 2, 20, 5, 19.99, '2024-12-09 16:28:07'),
(3, 3, 25, 3, 20.00, '2024-12-09 19:34:45'),
(4, 4, 1, 1, 450.00, '2024-12-10 16:13:10'),
(5, 7, 6, 1, 44.99, '2024-12-10 18:59:21'),
(5, 7, 17, 1, 50.00, '2024-12-10 18:59:21'),
(5, 7, 18, 1, 20.00, '2024-12-10 18:59:21'),
(6, 8, 8, 1, 124.99, '2024-12-10 19:01:53'),
(7, 9, 2, 1, 380.00, '2024-12-10 22:15:48'),
(7, 9, 16, 2, 99.99, '2024-12-10 22:15:48'),
(8, 10, 10, 4, 56.99, '2024-12-11 17:39:11'),
(9, 4, 19, 1, 7.99, '2024-12-11 19:44:47'),
(9, 4, 20, 1, 19.99, '2024-12-11 19:44:47');
...
--
-- Estructura Stand-in para la vista `product_views`
-- (Véase abajo para la vista actual)
--
...
--
-- Estructura Stand-in para la vista `shopping_cart_views`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `shopping_cart_views` (
`customer_id` int(11)
,`product_id` int(11)
...
--
-- Estructura para la vista `categories_views`
--
...
--
-- Estructura para la vista `product_views`
--
...
--
-- Estructura para la vista `shopping_cart_views`
--
DROP TABLE IF EXISTS `shopping_cart_views`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `shopping_cart_views`  AS SELECT `c`.`customer_id` AS `customer_id`, `p`.`product_id` AS `product_id`, `c`.`forename` AS `forename`, `c`.`surname` AS `surname`, `p`.`product_name` AS `product_name`, `s`.`quantity` AS `quantity`, `p`.`unit_price` AS `unit_price` FROM ((`shopping_cart` `s` join `customers` `c` on(`c`.`customer_id` = `s`.`customer_id`)) join `products` `p` on(`p`.`product_id` = `s`.`product_id`)) ;
