...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 05, 2025 at 09:22 PM
...
CREATE DEFINER=`root`@`localhost` FUNCTION `kg_to_lbs` (`var_weight_kg` INT) RETURNS INT(11) DETERMINISTIC BEGIN
	DECLARE var_weight_lbs INT;
    SET var_weight_lbs = ROUND(var_weight_kg * 2.2, 0);
    RETURN (var_weight_lbs);
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `language_compatibility` (`var_movie_id` INT, `var_character_id` INT, `var_actor_id` INT) RETURNS DECIMAL(10,0) DETERMINISTIC BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `skill_compatibility` (`var_movie_id` INT, `var_character_id` INT, `var_actor_id` INT) RETURNS DECIMAL(10,0) DETERMINISTIC BEGIN
		DECLARE var_skill_compatibility DECIMAL;
...
(2, 4, 3, 2.3, '2025-03-04 11:00:15'),
(2, 7, 15, 2.1, '2025-03-04 11:11:18');

--
-- Triggers `acting`
--
DELIMITER $$
CREATE TRIGGER `ins_tr_acting` AFTER INSERT ON `acting` FOR EACH ROW BEGIN
    INSERT INTO casting(movie_id, character_id, actor_id, casting_date, casting_position, is_hired, agreed_salary)
    VALUES (NEW.movie_id, NEW.character_id, NEW.actor_id, NEW.inserted_on, 1, 1, NEW.agreed_salary);
END
$$
DELIMITER ;
...
--
-- Table structure for table `actors_languages`
--
...
--
-- Dumping data for table `actors_languages`
--
...
(2, 4, 1, '2025-03-04 10:59:45', 3, 0, 2.5),
(2, 4, 3, '2025-03-04 11:00:15', 1, 1, 2.3),
(2, 4, 9, '2025-03-04 10:59:50', 2, 0, 2.3),
(2, 4, 11, '2025-03-04 10:59:55', 6, 0, 2.0),
(2, 4, 12, '2025-03-04 11:00:00', 4, 0, 1.9),
(2, 4, 15, '2025-03-04 11:00:07', 5, 0, 1.7),
(2, 5, 10, '2025-02-05 18:06:08', NULL, NULL, NULL),
(2, 5, 13, '2025-02-05 18:06:08', NULL, NULL, NULL),
(2, 7, 15, '2025-03-04 11:11:18', 1, 1, 2.1);

--
-- Triggers `casting`
--
DELIMITER $$
CREATE TRIGGER `del_tr_actor` AFTER DELETE ON `casting` FOR EACH ROW BEGIN
	DELETE FROM acting
...
--
-- Table structure for table `casting2`
--
...
CREATE TABLE `casting2` (
  ...
  `agreed_salary` decimal(2,1) DEFAULT NULL,
  `skill_compatibility` decimal(10,0) DEFAULT NULL,
...
--
-- Dumping data for table `casting2`
--
...
(2, 4, 1, '2025-03-05 20:14:03', 0.0, NULL, 67, 33, NULL, NULL),
(2, 4, 3, '2025-03-05 20:14:03', 2.3, NULL, 33, 67, NULL, NULL),
(2, 4, 5, '2025-03-05 20:14:03', 0.0, NULL, 100, 67, NULL, NULL),
(2, 4, 9, '2025-03-05 20:14:03', 0.0, NULL, 33, 100, NULL, NULL),
(2, 4, 11, '2025-03-05 20:14:03', 2.5, NULL, 67, 67, NULL, NULL),
(2, 4, 12, '2025-03-05 20:14:03', 0.0, NULL, 100, 67, NULL, NULL),
(2, 4, 15, '2025-03-05 20:14:03', 2.1, NULL, 100, 67, NULL, NULL),
(2, 6, 6, '2025-03-05 20:13:41', 0.0, NULL, 50, 33, NULL, NULL),
(2, 6, 14, '2025-03-05 20:13:41', 0.0, NULL, 75, 67, NULL, NULL);
...
--
-- Stand-in structure for view `casting_view_2`
-- (See below for the actual view)
--
...
(3, 'The Godfather', 'Francis Ford Coppola', 'drama', 6000000, 1972),
(4, 'Alien', 'Ridley Scott', 'horror', 11000000, 1979),
(5, 'Interstellar', 'Cristopher Nolan', 'drama', 165000000, 2014);
...
--
-- Structure for view `casting_view_2`
--
DROP TABLE IF EXISTS `casting_view_2`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `casting_view_2`  AS SELECT `casting`.`movie_id` AS `movie_id`, `casting`.`character_id` AS `character_id`, `casting`.`actor_id` AS `actor_id`, `casting`.`casting_position` AS `casting_position`, `casting`.`is_hired` AS `is_hired`, `casting`.`agreed_salary` AS `agreed_salary`, (select round(avg(`acting`.`agreed_salary`),1) from `acting` where `acting`.`actor_id` = `acting`.`actor_id` and year(`acting`.`inserted_on`) >= year(curdate()) - 3) AS `expected_salary` FROM `casting` ;
...
--
-- Indexes for table `actors_languages`
--
...
--
-- Indexes for table `characters_languages`
--
...
--
-- Constraints for table `actors_languages`
--
ALTER TABLE `actors_languages`
...
--
-- Constraints for table `casting2`
--
ALTER TABLE `casting2`
...
--
-- Constraints for table `characters_languages`
--
ALTER TABLE `characters_languages`
